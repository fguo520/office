  ...
  <TabelPageLayout :total="total" :pageNum.sync="listQuery.pageNum" :pageSize.sync="listQuery.pageSize" @request="getList" ref="tabelPageLayout">
    <ShrinkCard>
      <el-form :inline="true" size="mini" label-width="100px">
        <el-form-item label="自定义参数：">
          <el-select v-model="listQuery.type" placeholder="全部" size="mini" class="input-width" filterable style="width:100px">
      ...
      <template #button>
        ...
        <el-button @click="editor()" size="mini" v-if="rolePermissionJudge('/shopeePublish/publish')">刊登商品</el-button>
    ...
    <template #container>
      <u-table ref="tableContainer" style="width: 100%" :data="list" v-loading="listLoading" border height="100" use-virtual element-loading-text="数据加载中" :header-cell-style="{ 'background-color': '#dadde3' }" row-id="random" :default-expand-all="false" :treeConfig="{lazy: true, load:loadTableTree }" v-if="isDisplaymode">
        ...
        <u-table-column label="SKU" align="center" width="150" :tree-node="true">
  ...
  methods: {
    ...
    async getList() {
      ...
      try {
        ...
        this.showEditorButton = this.listQuery.status === 3 || this.listQuery.status === 4;
      } catch (error) {
        this.list = [];
        this.total = 0;
      }
    ...
    async handUser() {
      if (this.listQuery.userType == 1) {
        const res = await getUmsAdminList(0, "create");
        this.typeList.adminList = res.data
      } else {
        const res = await getUmsAdminList(0, "");
        this.typeList.adminList = res.data
      }
    },
    async getType() {
      const resAuthList = await getShopeePlatformAuthList();
      const resAdminList = await getUmsAdminList(0, "create");
    ...
    editor(row) {
      const query = row ? { routeSku: row.sku, routeTaskNo: row.taskNo, routeIsture: 'true', routeStatus: this.listQuery.status } : { routeIsture: 'true' };
    ...
    async copyPublishedDialog() {
      ...
      // const { sku, taskNo, storeName } = this.$refs.tableContainer.$refs.singleTable.selection[0];
