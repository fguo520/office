  ...
  <TabelPageLayout :total="total" :pageNum.sync="listQuery.pageNum" :pageSize.sync="listQuery.pageSize" @request="getList" ref="tabelPageLayout">
    ...
    <template #container>
      <u-table ref="tableContainer" style="width: 100%" :data="list" v-loading="listLoading" border height="100" use-virtual element-loading-text="数据加载中" :header-cell-style="{ 'background-color': '#dadde3' }">
        ...
        <u-table-column label="sku" align="center" width="165">
        ...
        <u-table-column label="销售数量" align="center" width="200">
          <template #default="{row}">
            <div style="text-align:left;">
        ...
        <u-table-column label="无法销售数量" align="center" width="200">
          <template #default="{row}">
            <div style="text-align:left;">
              <div v-for="(item, index) in getCantSalesNum" :key="index">
                <span class="spanWidth">{{ item.label }}：</span>
                <span v-copy="row[item.key]">{{ row[item.key] }}</span>
              </div>
            </div>
...
<script>
import { getLmAmazonAuthNode, getAmazonRecommendedRemovalList } from "@/api/amazon/amazonReport"
...
export default {
  ...
  computed: {
    getSalesNum() {
      return [
        { label: '121-180天', key: 'sellable121to180Days' },
        { label: '181-270天', key: 'sellable181to270Days' },
        { label: '271-365天', key: 'sellable271to365Days' },
        { label: '365天', key: 'sellable365Days' },
        { label: '数量', key: 'sellableQuantity' },
        { label: '销售移除数量', key: 'sellableRemovalQuantity' },
      ]
    },
    getCantSalesNum() {
      return [
        { label: '0-7天', key: 'unsellable0to7Days' },
        { label: '8-60天', key: 'unsellable8to60Days' },
        { label: '61-90天', key: 'unsellable61to90Days' },
        { label: '无法销售数量', key: 'unsellableQuantity' },
      ]
    }
  },
  methods: {
    async getList() {
      ...
      try {
        const res = await getAmazonRecommendedRemovalList(this.listQuery);
