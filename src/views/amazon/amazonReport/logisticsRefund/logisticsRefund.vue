  ...
  <TabelPageLayout :total="total" :pageNum.sync="listQuery.pageNum" :pageSize.sync="listQuery.pageSize" @request="getList" ref="tabelPageLayout">
    ...
    <template #container>
      <u-table ref="tableContainer" style="width: 100%" :data="list" v-loading="listLoading" border height="100" use-virtual element-loading-text="数据加载中" :header-cell-style="{ 'background-color': '#dadde3' }">
        ...
        <u-table-column label="sku" align="center" width="175">
        ...
        <u-table-column label="id" align="center" width="150">
          <template #default="{row}">
            <div style="text-align:left;">
              <el-tooltip class="item" effect="dark" :content="'订单id：'+row.amazonOrderId" placement="left">
                <div v-copy="row.amazonOrderId" class="tooltipClass">{{row.amazonOrderId}}</div>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" :content="'案例id：'+row.caseId" placement="left">
                <div v-copy="row.caseId" class="tooltipClass">{{row.caseId}}</div>
              </el-tooltip>
            </div>
          </template>
        </u-table-column>
        <u-table-column label="原始退款" align="center" width="150">
          <template #default="{row}">
            <div style="text-align:left;">
              <el-tooltip class="item" effect="dark" :content="'id：'" placement="left">
                <div v-if="row.originalReimbursementId" class="tooltipClass"><span v-copy="row.originalReimbursementId">{{row.originalReimbursementId}}</span></div>
              </el-tooltip>

              <el-tooltip class="item" effect="dark" :content="'类型：'" placement="left">
                <div v-if="row.originalReimbursementType" class="tooltipClass"><span v-copy="row.originalReimbursementType">{{row.originalReimbursementType}}</span></div>
              </el-tooltip>
            </div>
          </template>
        </u-table-column>
        <u-table-column label="价格 & 单位" align="center" width="110">
          <template #default="{row}">
            <div>{{ row.amountTotal }}</div>
            <div>{{ row.currencyUnit }}</div>
          </template>
        </u-table-column>
        ...
        <u-table-column label="站点 & 状态" align="center" width="100">
          <template #default="{row}">
            ...
            <div>{{ row.conditionList }}</div>
          </template>
        </u-table-column>
        <u-table-column label="退款信息" align="center" width="220">
          <template #default="{row}">
            <div style="text-align:left;">
              <div v-for="(item, index) in getRefundNum" :key="index">
        ...
        <u-table-column label="退款原因" align="center" width="200">
          <template #default="{row}">
            <div style="text-align:left;">
              <el-tooltip class="item" effect="dark" :content="row.reason" placement="top">
                <div class="tooltipClass">{{ row.reason }}</div>
              </el-tooltip>
            </div>
          </template>
        </u-table-column>
        <u-table-column label="退款时间" align="center" width="180">
          <template #default="{row}">
            <div>{{ row.approvalDate }}</div>
          </template>
        </u-table-column>
      </u-table>
    </template>
...
<script>
import { getLmAmazonAuthNode, getAmazonReimbursementList } from "@/api/amazon/amazonReport"
...
export default {
  ...
  computed: {
    getRefundNum() {
      return [
        { label: '退款id', key: 'reimbursementId' },
        { label: '退款现金总数', key: 'quantityReimbursedCash' },
        { label: '退款库存总数', key: 'quantityReimbursedInventory' },
        { label: '退款总数', key: 'quantityReimbursedTotal' },
      ]
    }
  },
  methods: {
    async getList() {
      ...
      try {
        const res = await getAmazonReimbursementList(this.listQuery);
