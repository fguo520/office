<template>
  <el-dialog title="导入" :visible.sync="visible" width="576px" append-to-body :before-close="close">
    <el-form ref="resourceForm" label-width="150px" size="mini" :model="listQuery" :rules="rules">
      <el-form-item label="获取模板：">
...
<script>
import { getTemplateUrl, importSensitiveWord } from "@/api/common/sysSensitiveword"
export default {
  name: "ExcelDialog",
  props: {
    visible: {
  ...
  methods: {
    ...
    async handleOk() {
      if (this.formData.file == "") {
        ...
        formData.append("platformCode", this.listQuery.platformCode);
        const res = await importSensitiveWord(formData)
        if (res.code == 200) {
