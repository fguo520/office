  ...
  <div class="elUploadContainer">
    <el-upload ref="upload" action="#" list-type="picture-card" :auto-upload="false" :multiple="true" :file-list="fileList" drag :on-change="handleChange" :limit="getLimit" :on-exceed="limitExceed" accept=".jpg,.png">
      ...
      <div slot="file" slot-scope="{file}">
        ...
        <span class="minWatermark" v-if="watermark.type == 1" :style="getMinWaterTextStyle">{{ watermark.text }}</span>
        <img class="minWatermark" v-if="watermark.type == 2" :src="watermark.imageUrl" :style="getMinWaterImgStyle">

        <span class="el-upload-list__item-actions">
          <span class="el-upload-list__item-preview" @click="handlePictureCardPreview(file)">
            <i class="el-icon-zoom-in"></i>
          </span>
          <span class="el-upload-list__item-replace" @click="handleReplace(file)">
    ...
    <el-dialog :visible.sync="dialogVisible" width="540px">
      <div class="maxwatermarkContaier">
        <img class="maxPicture" :src="dialogImageUrl" ondragstart="return false;" />
        <span class="maxwatermark" v-if="watermark.type == 1" :style="getMaxWaterTextStyle">{{ watermark.text }}</span>
        <img class="maxwatermark" v-if="watermark.type == 2" :src="watermark.imageUrl" :style="getMaxWaterImgStyle" ondragstart="return false;">
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { uploadImage } from "@/api/walmart/walmartEditor";
...
export default {
  ...
  computed: {
    getLimit() {
      return this.currentReplaceIndex ? 13 : 12
    },
  ...
  methods: {
    ...
    async handleChange(file, fileList) {
      ...
      if (!this.sku) {
        $message({ type: "warning", message: "请填入sku" })
        return
      }
      if (!regExcel.test(file.raw.name) && (file.size / 1024 / 1024 <= 1)) {
        $message({ type: "warning", message: "允许上传png/jpg图片格式,且图片大小于1MB" })
        return
      }
