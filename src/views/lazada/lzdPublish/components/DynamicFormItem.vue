<template>
  <el-form ref="form" :model="addAttrQuery" size="mini" label-width="150px" inline>
    <el-form-item v-for="(formItem,index) in addFormItemList" :key="formItem.name+formItem.id+index" :label="formItem.label" :prop="formItem.name" :rules="getRules(formItem)">
      ...
      <el-input v-model="addAttrQuery[formItem.name]" v-if="getInputType(formItem.inputType, 'textarea')" type="textarea" :autosize="{ minRows: 1, maxRows: 4}" style="width:200px;" placeholder="请输入内容" />

      <el-select v-model="addAttrQuery[formItem.name]" v-if="getInputType(formItem.inputType, 'select')" clearable filterable style="width:200px;">
        <el-option v-for="(item,index) in JSON.parse(formItem.options)" :key="item.name+index" :label="item.name" :value="item.name">
        </el-option>
      </el-select>

      <ScrollSelect v-model="addAttrQuery[formItem.name]" v-if="getInputType(formItem.inputType, 'multiSelect')" :prop="{label: 'name', value: 'name'}" :list="JSON.parse(formItem.options)" />
    </el-form-item>
  </el-form>
</template>
  ...
  computed: {
    ...
    getRules() {
      return function (item) {
        ...
        const mustBeRule = ["singleSelect", "enumInput", "multiSelect", "multiEnumInput"].indexOf(item.inputType)
