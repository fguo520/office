  ...
  <el-card>
    <el-form ref="form" :model="listQuery" label-width="150px" inline :rules="rules" size="mini">
      ...
      <el-form-item prop="shopId" label="店铺：">
        <el-select v-model="listQuery.shopId" placeholder="请选择店铺" filterable no-data-text="正在获取数据请稍后。。。" :disabled="!listQuery.isSpell" @change="handShopId" style="width:200px;">
          <el-option v-for="item in typeList.storeList" :key="item.id" :label="item.storeName" :value="item.id" />
        </el-select>
      </el-form-item><br>
      <el-form-item prop="sku" label="sku：">
        <el-input v-model="listQuery.sku" @change="skuChange" style="width:200px;" :disabled="!listQuery.isSpell" />
      </el-form-item><br>

      <el-form-item prop="title" label="标题：">
        <el-input v-model="listQuery.title" style="width:500px" @change="titleChange" maxlength="225" /><span style="margin-left:10px;color:#C0C4CC">{{maxlength+'/225'}}</span>
      </el-form-item><br>
      ...
      <el-form-item style="margin-left:40px;margin-right:40px;" class="table">
        <el-table ref="skuForm" :data="skuList" style="width: 100%;font-size:13px" border empty-text="输入正确sku和选择店铺后查询变体sku" size="mini" :header-cell-style="{ 'background-color': '#dadde3' }">
          ...
          <el-table-column prop="colorFamily" label="颜色" width="160" align="center">
            <template #default="{ row }">
              <!-- <el-select v-model="row.colorFamily" placeholder="暂无颜色" clearable filterable :no-data-text="listQuery.primaryCategory.length?'获取数据中...':'请先选择类目'" class="inptTextCenter">
                <el-option v-for="(item,index) in typeList.colorList" :key="item.name+index" :label="item.name" :value="item.name" />
              </el-select> -->
          ...
          <el-table-column prop="date" label="刊登促销价格" width="140" align="center">
            <template #default="{ row }">
              <el-popover placement="bottom">
                <div v-for="item in row.prices" :key="item.publishedTo">
                  <div v-if="publishedForPriceShow(item.publishedTo)" class="priceDetail">
                    <div>
                      <span class="label">{{ item.publishedTo }}：</span>
                      <span>{{ getFormula(item.publishedTo, row, item).calculate }}</span>
      ...
      <!-- <el-form-item prop="autoTranslatedTo" label="自动翻译：">
        <el-checkbox-group v-model="listQuery.autoTranslatedTo">
          <el-checkbox label="Thai">泰语</el-checkbox>
          <el-checkbox label="Bahasa Indonesia">马来西亚语</el-checkbox>
          <el-checkbox label="Bahasa Malaysia">印度尼西亚语</el-checkbox>
          <el-checkbox label="Vietnamese">越南语</el-checkbox>
        </el-checkbox-group>
      </el-form-item><br> -->
      ...
      <div style="text-align:center; margin-bottom:150px;">
        <el-button @click="onSubmit(0)" :loading="saveButtonLoding">保存至草稿</el-button>
...
export default {
  ...
  watch: {
    ...
    // "listQuery.shopId"(newValue, oldValue) {
    //   if (!this.listQuery.isSpell) return;
    //   const newName = getStoreName.call(this, newValue)? `${getStoreName.call(this, newValue)} `: "";
    //   const oldName = getStoreName.call(this, oldValue)? `${getStoreName.call(this, oldValue)} `: "";
    //   if (this.listQuery.title.startsWith(oldName)) {
    //     this.listQuery.title = this.listQuery.title.replace(oldName, newName);
    //     return;
    //   }
    //   this.listQuery.title = `${newName}${this.listQuery.title}`;
    // },
    'listQuery.title': function (newValue) {
  ...
  methods: {
    async getSearchType() {
      if (this.routeSku) {
        // 修改商品时获取所有的产品属性
        const res = await getLmLazadaApiProductUpdate({
          sku: this.routeSku,
    ...
    async onSubmit(butSave) {
      try {
        ...
        const pictureValid = this.skuList.every(item => item.images.length >= 3);
        if (!pictureValid) {
          $message.warning(`请为每个变体sku插入至少3张图片`)
          return;
        }
        const quantityValid = this.skuList.every(item => item.quantity > 0);
        if (!quantityValid) {
